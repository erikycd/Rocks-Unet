...
def get_tensor(img_path, mask_path, IMG_HEIGHT , IMG_WIDTH):
    ...
    for i in range(0,N_Img):
        ...
        A = cv.imread(img_path + Images[i] , cv.IMREAD_UNCHANGED)
        A = (A - np.mean(A))/(np.std(A))
        A = cv.resize(A, (IMG_HEIGHT, IMG_WIDTH), interpolation = cv.INTER_NEAREST)

        B = cv.imread(mask_path + Mask[i])[:,:,0]
        B = cv.resize(B, (IMG_HEIGHT, IMG_WIDTH), interpolation = cv.INTER_NEAREST)
        B = B[:,:,np.newaxis]
